...
\end{theorem}
\begin{proof}
	see p.20 of \cite{Etingof} from the 2009 lecture notes to "Topics in Lie Theory".
...
\begin{definition}[planar operad, symmetric operad]
	...
	such that 1. and 2. of following are satisfied. If each object $P(n)$ has a right $\symgrp_n$-action ($\symgrp_n$ being the $n$th symmetric group), and 3. is staisfied, then $P$ is a \emph{symmetric operad}, which is what "operad" refers to from now on.
	\begin{enumerate}
...
\end{tikzcd}\]
		\item Equivariance: the right action $-\cdot\sigma:P(n)\to P(n)$ of $\sigma\in \symgrp_n$ on $P(n)$ is given by $\sigma$ permuting the $n$ inputs of each "element" of $P(n)$, so that on operadic composition maps, we have
...
(where $(-)\sigma$ is the isomorphism given by $\sigma$ permuting the $n$ factors of $\bigotimes_{i=1}^nP(m_i)$)
	\end{enumerate}
\end{definition}
\begin{example}[the endofunctor operad]
	If $\sC$ is a category, then the endofunctor operad, defined by $\End(\sC)(n)\defeq\Fun\left( \sC^{\times n},\sC \right)$ for $n\in\bN_{\ge0}$, is a symmetric operad in $(\Cat,\times,\{*\})$, with 
	\begin{enumerate}
	\item $\id_\sC$ as the unit, and $\times$ as the monoidal product.
	\item the standard composition as the operadic composition,
	\item $\sigma\in \symgrp_n$ acting on each element $\theta\in\Fun(\sC^{\times n},\sC)$ by permuting the inputs, i.e. $\theta\cdot\sigma(x_1,\cdots,x_n)=\theta(x_{\sigma^{-1}(1)},\cdots,x_{\sigma^{-1}(n)})$.
	\end{enumerate}
\end{example}
The endofunctor operad is a special case of the endomorphism operad ; it is an endomorphism operad in $\left( \Cat,\times,\left\{ * \right\} \right)$, a symmetric monoidal category with internal hom.

\begin{example}[the operad of parenthesized braids] \label{PaBdefinition}
	This construction is similar to the one in pp.18-19 of \cite{Fresse_2018}. $\PaB$ is an operad in $\mathbf{Grpd}$ (the category of groupoids).
	
	The objects of $\PaB(n)$ are parenthesized permutations of $n$ elements, i.e.
	\[\Obj(\PaB(n))=\symgrp_n\times M_n,\]
	where $M_n$ is the set of words of length $n$ in the free magma generated by one element. If $(\sigma,w)\in\PaB(n)$, then for $1\le i\le n$, we label the $i$th factor of $w$ (which must be the generating element) with $\sigma(n)$ to get the parenthesized permutation.

	The morphisms of $\PaB(n)$ are braids from $(\sigma,w)$ to $(\tau,v)$ such that if the $l$ factor of $w$ and the $m$th factor of $v$ are the ends of a strand, then $\sigma(l)=\tau(m)$.

	$\PaB$ has the following operadic properties:
	\begin{enumerate}
		\item The unit, which happens to be the only morphism of $\PaB(1)$, is the braid from $(1,x)$ to $(1,x)$.
		\item The operadic composition is given by cabling:
		\item The action of $\symgrp_n$ on $\PaB(n)$ is given by $(\sigma,w)\overset{\tau}{\mapsto}(\tau\sigma,w)$ on objects.
	\end{enumerate}
	% Each element of $\PaB(n)$ is equivalent to a rooted binary tree with its $n$ leaves labled by a permutation of $\{1,\cdots,n\}$
\end{example}
...
\begin{definition}[operad morphism]
	...
	\begin{enumerate}
		...
		\end{tikzcd}\]
		\item The $\symgrp_n$-actions are preserved, i.e. for all $n\in\bN_{\ge1}$, $\sigma\in \symgrp_n$, $f\circ(-\cdot^P\sigma)=(-\cdot^Q\sigma)\circ f$.
	\end{enumerate}
\end{definition}
\begin{definition}[operad algebra]
	For an operad $P$ in $(\Cat,\times,\{*\})$, a morphism of operads $P\to\End(\sC)$ gives $\sC$ the structure of a \emph{$P$-algebra}.
\end{definition}